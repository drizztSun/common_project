...
class BestTimeToBuySellStockIV {
	...
	*/
	// <DP>
	int doit_dp_bottomup(int k, vector<int>& prices) {

        int n = prices.size();
		if (n < 2) return 0;
		
		int res = 0;
        // k more then half of the array, add all profit.
		if (k >= n / 2) {
			for (int i = 1; i < n; i++)
		...
		for (int x = 1; x < k + 1; x++) {
			...
			for (int i = 1; i < n; i++) {
				...
				DP[x][i] = std::max(DP[x][i - 1], prices[i] + tmp);
				tmp = std::max(tmp, DP[x - 1][i] - prices[i]);
				maxv = std::max(maxv, DP[x][i]);
			}
...
public:
    ...
    /*
        ...
        我们在令g(x)=f(x)-a*x，从形式上看，就是一个带有transaction fee的股票买卖问题，其中每买一次股票需要额外付出a。特别地，当a=0的时候，g(x)就是f(x)。我们此时考虑，当a=1的时候相比于a=0的时候，g(x)的函数图像有什么变化呢？首先对任何的交易次数x而言，交易费的引入都会让利润受损，所以g(x)整体会向下移动。另外，交易费的引入会让有些交易变得无利可图甚至亏损，最大利润所对应的股票交易次数一定会减少，所以g(x)的峰值会左移。

        可以想见，随着a的增大，g(x)的峰值会向下向左移动。注意，x的定义域是离散的，所以峰值也一定对应着某个整数的x。

        如果我们能找到某个a，使得g(x)的峰值恰好对应着x=k的时候，意味着f(k)-ak = g(x)_max. 如果g(x)_max和a都知道了，那么本题就得解。那么如何知道这样的a呢？事实上，我们已经知道，随着a的增大，g(x)的峰值位置是向左单调移动的。我们可以用二分法搜索出这个a来，满足峰值是的位置是k。此时交易费a确定了，那么问题就同714 Best Time to Buy and Sell Stock with Transaction Fee，可以知道最大利润，也就是g(x)的峰值。
...
public:
    ...
    int maxProfit(int k, vector<int>& prices) 
    {
        ...
        int left = 0, right = *std::max_element(prices.begin(), prices.end());
...
public:
	...
	*/
	...
	int doit1(int k, vector<int>& prices) {
		...
		stack<std::pair<int, int>> vp_pairs;
