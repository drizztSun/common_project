...
class FallingSquares {
    ...
    vector<int> doit_hashtable_2(vector<vector<int>>& positions) 
    {
        map<int,int>Map;
        Map[0]=0;
        Map[INT_MAX]=0;
...
public:
    ...
    struct segmentnode {
        ...
        segmentnode(int s = 0, int e = 1e9, int h = 0) {
            start = s;
            end = e;
            height = h;
            left = NULL;
        ...
        int getstatus(int s, int e) {

            if (s <= start && e >= end) return height;
            if (s >= end || e <= start) return 0;
            if (left == NULL) return height;
            return std::max(left->getstatus(s, e), right->getstatus(s, e));
        }

        void updatestatus(int s, int e, int h) {
            ...
            if (left == NULL) {
                ...
                left = new segmentnode(start, mid, height);
                right = new segmentnode(mid, end, height);
            }

            left->updatestatus(s, e, h);
            right->updatestatus(s, e, h);
    ...
    vector<int> fallingSquares(vector<vector<int>>& positions) {
        ...
        for (auto c : positions) {
            ...
            int s = c[0], e = c[0] + c[1], h = c[1];

            int height = root.getstatus(s, e);

            curmax = std::max(curmax, h + height);

            root.updatestatus(s, e, h + height);
