...
/*    
    ...
    如果这条直系链的某个祖先有一条分叉，那么我们发现原来的直系链上的节点，从该祖宗以上的部分的degree都会增加（因为它们有其他更多的直系子孙了）。这就告诉我们，对于同一条直系链上的节点，从高到低的degree必然是下降/不增的。
    所以我们如果把relative[x]的所有节点按照degree从大到小排列，那么x的父节点应该就是degree恰好大于或等于x的那个节点。当然，这样的节点可能会有多个，我们可以强制选定其中一个，因为之前说过，同一个直系链里面degree相同的节点可以互换。但是注意，这种“强制”必须是全局一致的，举个例子

        a
        b
        c  e
    d      f
    在直系链a-b-c-d中，a和b的度相等，如果我们制定c的父节点是b的话，那么在直系链a-b-e-f中，e的父节点就不能随意指定，必须也是b而不能是a。为了保证这种“全局一致性”，我们需要将全部节点nodes按照度来从小到大排序编号，
    比如说上面的例子就是"dcfeba"，我们规定属于同一个直系链、度相同的两个节点，排序靠前的节点更靠下层。这样对于c而言，它有两个度相同的祖先a和b，根据规则b是它的父亲。同理对于e而言，也可以判定b是它的父亲，而不是a。
