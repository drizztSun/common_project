...
class DivideIntoSubsequences {
    ...
    /*
        ...
        Find the number of groups as in the solution, assume it equals to M
        Assign A[i] to groups[i % M].
        Done.
   ...
   bool doit_greedy(vector<int>& A, int K) {
        int cur = 1, groups = 1, n = A.size();
        for (int i = 1; i < n; ++i) {
            cur = A[i - 1] <  A[i] ? 1 : cur + 1;
            groups = std::max(groups, cur);
        }
        return n >= K * groups;
    }
};