...
public:
    ...
    int doit_search(int N, vector<vector<int>>& mines) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                grid[i][k] = std::min(grid[i][k], right);
                
                bottom = grid[j][i] != 0 ? (bottom + 1) : 0;
                grid[j][i] = std::min(grid[j][i], bottom);
                
                up = grid[k][i] != 0 ? (up + 1) : 0;
                grid[k][i] = std::min(grid[k][i], up);
            }
        }
