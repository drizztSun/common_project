...
class NumArray {
    ...
    public:
        ...
        segtree(int s, int e, int val=0): _begin(s), _end(e), _val(val), _left(nullptr), _right(nullptr) {}
    };
    
    segtree* build(int s, int e) {
        
        if (s > e) return nullptr;
        
        segtree *node = new segtree(s, e);
        ...
        if (!node->_left) {
            int mid = (s+e)/2;
            node->_left = build(s, mid);
            node->_right = build(mid+1, e);
        }
        
        node->_val = node->_left->_val + node->_right->_val;
        return node;
    } 
    
    void _update(segtree* node, int i, int val) {
        
        
        if (node->_begin == i && node->_end == i) {
            node->_val = val;
            return;
        }
        
        int mid = (node->_begin + node->_end) / 2;
        if (mid < i) {
            _update(node->_right, i, val);
        } else {
            _update(node->_left, i, val);
        }
    ...
    int query(segtree* node, int s, int e) {
        ...
        if (node->_begin > e || node->_end < s) return 0;
        
        if (node->_begin >= s && node->_end <= e) {
            return node->_val;
        }
        
        return query(node->_left, s, e) + query(node->_right, s, e);
    }
...
public:
    NumArray(vector<int>& nums) {
        ...
        _nums = nums;
        _root = build(0, _nums.size()-1);
    }
    
    void update(int index, int val) {
        _update(_root, index, val);
    }
