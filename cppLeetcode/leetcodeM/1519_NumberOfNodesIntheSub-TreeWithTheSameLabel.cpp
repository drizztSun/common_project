...
public:
    vector<int> countSubTrees(int n, vector<vector<int>>& edges, string labels) {
        ...
        std::function<unordered_map<char, int>(int, int)> dfs = [&](int node, int parent) -> unordered_map<char, int> {
            ...
            for (auto c : graph[node]) {
                if (c != parent) {
                    unordered_map<char, int> childcounter = dfs(c, node);
                    for (auto item: childcounter) {
                        auto it = counter.find(item.first);
                        if (it != counter.end()) {
                            counter[it.frist] = it.second};
                        } else {
