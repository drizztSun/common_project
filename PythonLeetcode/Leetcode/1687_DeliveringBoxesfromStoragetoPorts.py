...
class DeliverBoxes:
    ...
    def doit_slidingwindow(self, boxes: list, portsCount: int, maxBoxes: int, maxWeight: int) -> int:

        N = len(boxes)
        boxes.insert(0, [-1, 0])
        lastj, lastport = 0, -1
        travels, totalWeight = 0, 0
        dp = [float('inf')] * (1+N)
        dp[0] = 0
        j = 0
        ...
        for i in range(1, N+1):
            ...
            while j+1 <= N and j+1-i+1 <= maxBoxes and boxes[j+1][1] + totalWeight <= maxWeight:
                j += 1
                ...
                if (boxes[j][0] != lastport):
                    lastj, lastport = j, boxes[j][0]

            dp[j] = min(dp[j], dp[i-1] + travels + 1)

            if j+1 <= N and boxes[j][0] == boxes[j+1][0]:
                dp[lastj - 1] = min(dp[lastj-1], dp[i-1] + travels)


            totalWeight -= boxes[i][1]
            travels -= (i+1 <= N and boxes[i][0] != boxes[i+1][0])
