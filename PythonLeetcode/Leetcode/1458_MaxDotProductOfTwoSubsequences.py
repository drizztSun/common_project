...
class MaxDotProduct:
    ...
    def doit_(self, nums1: list, nums2: list) -> int:
        ...
        minv1, maxv1 = float('inf'), float('-inf')
        ans = float('-inf')

        for i in range(1, m+1):

            minv1 = min(minv1, nums1[i])
            maxv1 = max(maxv1, nums1[i])

            for j in range(1, n+1):
                onlyone = max(minv1 * nums2[j], maxv1 * nums2[j])
                product = max(0, nums2[j] * nums1[i])

                dp[i][j] = max(dp[i-1][j], dp[i][j-1], dp[i-1][j-1] + product, onlyone)
