...
class CollisionTimes:
    ...
    def doit_monotonic_queue(self, cars: list) -> list:
        ...
        for i in reversed(range(n)):
            ...
            while st:
                ...
                lastp, lastspeed = cars[st[-1]]
                
                if lastspeed >= speed or float(lastp - pos) / float(speed - lastspeed) > res[st[-1]] and res[st[-1]] > 0:
                    st.pop()
                else:
                    break
            
            if st:
                res[i] = float(cars[st[-1]][0] - pos) / float(speed - cars[st[-1]][1])
