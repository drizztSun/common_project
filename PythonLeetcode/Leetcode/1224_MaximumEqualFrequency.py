...
class MaximumEqualFrequency:
    ...
    def doit_(self, nums: list) -> int:
        ...
        for i, c in enumerate(nums, 1):
            ...
            if freq[f] * f == i and i < len(nums):
                ans = max(ans, i + 1)

            d = i - freq[f] * f
            if (d == f + 1 or d == 1) and freq[d] == 1:
                ans = i
...
if __name__ == '__main__':
    ...
    MaximumEqualFrequency().doit_([1,1,1,2,2,2,3,3,3,4,4,4,5])

    MaximumEqualFrequency().doit_([2,2,1,1,5,3,3,5])