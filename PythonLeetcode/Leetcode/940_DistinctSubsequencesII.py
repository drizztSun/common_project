...
class DistinctSubseqII:
    ...
    Let's try for a dynamic programming solution. In order to not repeat work, our goal is to phrase the current problem in terms of the answer to previous problems.
    ...
    We have that dp[3]countsdp[2], plus("b", "aa", "ab", "ba", "aba")with"b"added to it. Notice that("", "a")are missing from this list, as they get double counted.
...
if __name__ == '__main__':
    ...
    res = DistinctSubseqII().doit("zchmliaqdgvwncfatcfivphddpzjkgyygueikthqzyeeiebczqbqhdytkoawkehkbizdmcnilcjjlpoeoqqoqpswtqdpvszfaksn")

    res = DistinctSubseqII().doit("abc")

    res = DistinctSubseqII().doit("aba")

    res = DistinctSubseqII().doit("aaa")
