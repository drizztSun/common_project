...
class MincostToHireWorkers:
    ...
    (This worker will be the 'captain', as described in Approach #1.)
    ...
    let's calculate the cost of hiring K workers where each point of quality is worth wage[captain] / quality[captain] dollars.
    ...
    Time Complexity: O(N^2 \log N), where NN is the number of workers.
    Space Complexity: O(N).
    """
...
if __name__ == '__main__':
    ...
    res = MincostToHireWorkers().doit(quality=[10, 20, 5], wage=[
        70, 50, 30], K=2)  # 105.00000

    res = MincostToHireWorkers().doit(quality=[3, 1, 10, 10, 1], wage=[
        4, 8, 2, 2, 7], K=3)  # 30.66667
