...
class LeastOpsExpressTarget:
    ...
    Let's think of the cost of a block as all the operators needed to express it, including the addition or subtraction operator in front of it.
    For example, we can think of x * x + x + x / x as (+ x * x) (+ x) (+ x / x) for a cost of 2 + 1 + 2, minus 1 for the leading + (so the total cost is 4).

    We can write the cost of writing a block that has value x^e : it is ee, except when e = 0e=0 it is 2. We want the sum of the costs of all blocks minus 1.
    ...
    Notice that modulo xx, the only blocks that change the expression are x^0. Let r_1 = target(modx). So we must either subtract r_1 * x^0 * s, or add x - r_1 * x^0x * 's.
    ...
    However, since the new target is divisible by xx, there is no point to use x^0, as we would have to use at least x of them
    to do the same work as one use of x^1, which is a strictly higher cost.

    Again, in a similar way, we have r_2 = target(mod x^2), and we must either subtract r_2 / xr
2
​
 /x x^1x
1
