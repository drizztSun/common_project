...
class BusiestServers:
    ...
    def doit_heap(self, k: int, arrival: list, load: list) -> list:

        from heapq import heappush, heappop
        from bisect import insort, bisect_left, bisect_right
        tasks = [0 for _ in range(k)]
        idles_server = [i for i in range(k)]
        buff = []

        for i in range(len(arrival)):

            s, e = arrival[i], arrival[i] + load[i]

            while buff and buff[0][0] <= s:
                idx = heappop(buff)[1]
                insort(idles_server, idx)

            if not idles_server:
                continue

            idx = bisect_left(idles_server, i % k)
            if idx == len(idles_server):
                idx = 0

            tasks[idles_server[idx]] += 1
            heappush(buff, [e, idles_server[idx]])
            idles_server.pop(idx)

        c = max(tasks)
        return [i for i in range(len(tasks)) if tasks[i] == c]

    def doit_(self, k, arrival, load):

        n = len(arrival)
        before, after, running_jobs = list(range(k)), [], []
        handled_req = [0] * k
        for i in range(n):
            server_id = i % k
            if server_id == 0:
                after, before = before, []
            while running_jobs and running_jobs[0][0] <= arrival[i]:
                _, server = heappop(running_jobs)
                if server < server_id:
                    heappush(before, server)
                else:
                    heappush(after, server)
            free_servers = after if after else before
            if not free_servers:
                continue
            server = heappop(free_servers)
            handled_req[server] += 1
            heappush(running_jobs, (arrival[i] + load[i], server))
        busiest = max(handled_req)
        return [server for server in range(k) if handled_req[server] == busiest]


if __name__ == '__main__':

    BusiestServers().doit_(3, [1,2,3,4,5], [5,2,3,3,3])
