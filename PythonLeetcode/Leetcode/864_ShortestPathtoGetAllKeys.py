...
This means that there is exactly one key for each lock, and one lock for each key; and also that the letters used to represent the keys and locks were chosen
...
class ShortestPathAllKeys:
    ...
    We have to pick up the keys K in some order, say Kσ.
    ...
    Between each segment of our path (and corresponding breadth-first search), we should remember what keys we've picked up.
    Keys that are picked up become part of a mask that helps us identify what locks we are allowed to walk through during the next breadth-first search.
    ...
    """
    def doit_brute_force(self, grid):
        import itertools
        import collections
    ...
    Let's make this intuition more formal: any walk can be decomposed into primitive segments, where each segment (between two points of interest) is primitive
    if and only if it doesn't touch any other point of interest in between.
    ...
    Afterwards, we have some graph (where each node refers to at most 1313 places, and at most 2^6 states of keys).
    We have a starting node (at '@' with no keys) and ending nodes (at anywhere with all keys.) We also know all the costs to go from one node to another -
      each node has outdegree at most 13. This shortest path problem is now ideal for using Dijkstra's algorithm.

    Dijkstra's algorithm uses a priority queue to continually searches the path with the lowest cost to destination, so that when we reach the target,
    we know it must have been through the lowest cost path. Refer to this link for more detail.
    ...
    """
    def doit_dijkstra_heap(self, grid):
        import heapq
        import collections
        R, C = len(grid), len(grid[0])
...
if __name__ == '__main__':
    ...
    #ShortestPathAllKeys().doit_brute_force(["@.a.#",
    #                                         "###.#",
    #                                         "b.A.B"])

    ShortestPathAllKeys().doit_dijkstra_heap(["@.a.#",
                                             "###.#",
                                             "b.A.B"])

    ShortestPathAllKeys().doit_dijkstra_heap(["@..aA",
                                             "..B#.",
                                             "....b"])