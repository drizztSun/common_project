...
class SplitStringIntoUniqueSubstrings:
    ...
    def doit_dfs_backtracking(self, s: str) -> int:

        buff = set()
        ans = 0

        def dfs(cutidx, length):
            ...
            for i in range(cutidx, len(s)):

                if s[cutidx: i+1] not in buff:

                    buff.add(s[cutidx:i+1])
                    dfs(i+1, length+1)
                    buff.remove(s[cutidx:i+1])
