...
class CoordinateWithMaximumNetworkQuality:
    ...
    def doit_(self, towers: list, radius: int) -> list:
        from math import dist
        from functools import product

        f = lambda i, j: sum(int(q / (1 + dist((i, j), (x, y)))) for x, y, q in towers if dist((i, j), (x, y)) <= radius)
    
        return max(product(range(51), range(51)), key=lambda x:(f(*x), [-x[0], -x[1]]))