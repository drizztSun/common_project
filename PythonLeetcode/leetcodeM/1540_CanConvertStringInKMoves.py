...
class CanConvertString:
    ...
    def doit_(self, s: str, t: str, k: int) -> bool:
        ...
        moves = [0] * 27
        for i in range(26):
            moves[i] = n if (i > r) else (n+1)
        
        needs = [0] * 27
        for i in range(len(s)):
            needs[(26 + ord(t[i])- ord(s[i])) % 26] += 1
            
        return all( moves[i] >= needs[i] for i in range(1, 27) if needs[i] > 0)


if __name__ == '__main__':

    CanConvertString().doit_("input", "ouput", 9)