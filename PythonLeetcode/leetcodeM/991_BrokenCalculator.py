...
class BrokenCalc:
    ...
    If say Y is odd, then if we perform 3 additions and one division, we could instead perform 1 addition,
    1 division, and 1 addition for less operations [(Y+3) / 2 vs (Y+1) / 2 + 1].
...
if __name__ == '__main__':
    ...
    res = BrokenCalc().doit(X=2, Y=3)

    res = BrokenCalc().doit(X=5, Y=8)

    res = BrokenCalc().doit(X=3, Y=10)

    res = BrokenCalc().doit(X=1024, Y=1)
