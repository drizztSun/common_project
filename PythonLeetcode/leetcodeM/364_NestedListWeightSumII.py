...
class NestedListWeightSumII:
    ...
    def doit_stack(self, nestedList: list) -> int:
        ...
        def search(cur, depth):
            ...
            for c in cur:
                if c.isInteger():
                    numslist.append((c.getInteger(), depth))
                    continue
                search(c.getList(), depth+1)

        search(nestedList, 1)

        return sum(num * (maxdepth - dep + 1) for num, dep in numslist)
