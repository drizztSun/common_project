...
class BulbSwitcherII:
    ...
    """
        ...
        As the search space is very large (2^N states of lights, naively 4^M4 operation sequences), let us try to reduce it.

        The first 6 lights uniquely determine the rest of the lights. This is because every operation that modifies the xx-th light also modifies the (x+6)-th light.
        ...
        Because c_i	≡ f_i and c_i ≤ f_i, if \sum f_i \not\equiv \sum c_i∑f 
        i
        ​	
        ...
        i
        ​	
        , or if \sum f_i < \sum c_i∑f 
        i
        ​	
        <∑c 
        i
        ​	
        , it isn't possible. Otherwise, it is possible by a simple construction: do the operations specified by c_ic 
        i
        ​	
        , then do operation number 1 with the even number of operations you have left.
    ...
    def doit_(self, n: int, m: int) -> int:
