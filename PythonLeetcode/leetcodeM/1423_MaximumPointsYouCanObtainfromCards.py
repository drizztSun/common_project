...
class MaxPointsYouCanObtain:
    ...
    def doit_(self, cardPoints: list, k: int) -> int:
        ...
        total = 0
        for i in reversed(range(len(cardPoints))):
            total += cardPoints[i]
            if len(cardPoints) - i == k:
                break
    ...
    def doit_(self, cardPoints: list, k: int) -> int:
        ...
        leftpointer, rightpointer = k-1, len(cardPoints)-1
        while leftpointer >= 0:
            leftSum -= cardPoints[leftpointer]
            rightSum += cardPoints[rightpointer]
            
            total = max(total, leftSum + rightSum)
            leftpointer -= 1
            rightpointer -= 1
...
if __name__ == '__main__':
    ...
    MaxPointsYouCanObtain().doit_([1,79,80,1,1,1,200,1], 3)
