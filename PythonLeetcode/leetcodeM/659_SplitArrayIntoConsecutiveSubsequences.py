...
class SplitArrayIntoConsecutiveSubsequences:
    ...
    Time Complexity: O(N), where NN is the length of nums. We iterate over the array and every event is added or popped to starts at most once.

    Space Complexity: O(N), the size of starts.
    """

    def do_(self, nums: 'list') -> bool:
        import collections
        import itertools
    ...
    Why? If we started with numbers x and greater from the beginning, the shorter chains starting from x could be concatenated with the chains ending before x, 
...
if __name__ == '__main__':
    ...
    SplitArrayIntoConsecutiveSubsequences().doit_greedy([1,2,3,3,4,5])

    SplitArrayIntoConsecutiveSubsequences().doit_greedy([1,2,3,3,4,4,5,5])

    SplitArrayIntoConsecutiveSubsequences().doit_greedy([1,2,3,4,4,5])
