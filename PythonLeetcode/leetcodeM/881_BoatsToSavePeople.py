...
class NumRescueBoats:
    ...
    Then, as described above, if the heaviest person can share a boat with the lightest person (if people[j] + people[i] <= limit) then do so;
    otherwise, the heaviest person sits in their own boat.
    """
    def doit(self, people, limit):
...
if __name__ == '__main__':
    ...
    res = NumRescueBoats().doit(people = [1,2], limit = 3) # 1

    res = NumRescueBoats().doit(people = [3,2,2,1], limit = 3) # 3

    res = NumRescueBoats().doit(people = [3,5,3,4], limit = 5) # 4
