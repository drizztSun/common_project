...
class MaximumOperationMaxProfit:

    def doit_(self, customers: list, boardingCost: int, runningCost: int) -> int:

        bestprofit, waitlist, onboard, rotated, times, bestround = float('-inf'), 0, 0, 0, 0, 0

        def oneRotate():
            nonlocal bestprofit, waitlist, onboard, rotated, bestround, times
            v = min(4, waitlist)
            times += 1
            waitlist -= v
            onboard += v
            rotated += 1
            curprofit = boardingCost * onboard - rotated * runningCost
...
if __name__ == '__main__':
    ...
    MaximumOperationMaxProfit().doit_([8, 3], 5, 6)
