...
class DisplayTable:
    ...
    def doit_(self, orders: list) -> list:

        from collections import defaultdict
        foods = set()
        tables = defaultdict(lambda : defaultdict(int))
        totalorders = defaultdict(int)
        
        for _, tbNum, food in orders:
            tables[tbNum][food] += 1
            foods.add(food)
            totalorders[tbNum] += 1
        ...
        foods = list(foods)
        foods.sort()
        totalindex = list(map(lambda x: int(x), totalorders.keys()))
        totalindex.sort()
        
        ans = []
        ans.append(["Table"] + foods)
        for c in totalindex:
            ans.append([str(c)] + [str(tables[str(c)][f]) for f in foods])
