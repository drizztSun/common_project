...
class ClosestCost:
    ...
    def doit_(self, baseCosts: list, toppingCosts: list, target: int) -> int:
        ...
        costs = set(baseCosts)
        candidates = set(baseCosts)

        diff, cloestval = float('inf'), 0

        for c in baseCosts:
            if abs(c - target) < diff:
                diff, cloestval = abs(c - target), c
        ...
        for t in toppingCosts:
            ...
            for cost in costs:
                ...
                for i in range(1, 3):
                    ...
                    if c < target + diff: candidates.add(c)

                    if abs(c-target) < diff or abs(c-target) == diff and c < cloestval:

                        cloestval, diff = c, abs(c-target)
        ...
        for top in toppingCosts:
            for _ in range(2):
                for i in reversed(range(n + 2)):
                    if i - top > 0 and dp[i - top]:
                        dp[i] = True

        ans = float('inf')
        # print(dp)
        for i in range(n + 2):
            if dp[i] and abs(i - target) < abs(ans - target):
                ans = i
