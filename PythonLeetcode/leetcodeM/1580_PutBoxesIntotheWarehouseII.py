...
class MaxBoxesInWarehouseII:
    ...
    def doit_greedy(self, boxes: list, warehouse: list) -> int:
    ...
    def doit_(self, boxes: list, warehouse: list) -> int:
        boxes.sort(reverse=True)
        Lw, Lb = len(warehouse), len(boxes)
        w_left, w_right = 0, len(warehouse) - 1
        
        for box in boxes:
            if warehouse[w_left] >= box:
                w_left += 1
            elif warehouse[w_right] >= box:
                w_right -= 1
            if w_left > w_right:
                return Lw
