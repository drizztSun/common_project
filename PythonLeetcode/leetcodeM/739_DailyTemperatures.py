...
class DailyTemperatures:
    ...
    def doit_stack(self, T: list) -> list:
    ...
    """
        ...
        Space Complexity: O(N + W)O(N+W), where NN is the length of T and WW is the number of allowed values for T[i]. Since W = 71W=71, we can consider the space complexity as O(N)O(N).

    """
    def doit_nextarray(self, T):
        nxt = [float('inf')] * 102
        ans = [0] * len(T)
