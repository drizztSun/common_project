...
class MaxIncreaseKeepingSkyline:
    ...
    def doit(self, grid):
        row_max = [max(c) for c in grid]
        col_max = [max(c) for c in zip(*grid)]

        return sum(min(row_max[x], col_max[y]) - grid[x][y] for x in range(len(grid)) for y in range(len(grid[0])))

    def doit(self, grid):
        cnt = 0
        max_v = [max(row) for row in grid]
        max_h = [max(col) for col in zip(*grid)]

        for i in range(len(grid)):
            for j in range(len(grid[0])):
                cnt += min(max_v[i], max_h[j]) - grid[i][j]
...
if __name__ == '__main__':
    ...
    res = MaxIncreaseKeepingSkyline().doit(
