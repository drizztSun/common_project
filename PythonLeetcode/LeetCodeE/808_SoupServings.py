...
we have quantities of soup represented by the state (x, y), and we can either go to (x-4, y-0), (x-3, y-1), (x-2, y-2), or (x-1, y-3) each with equal probability.
...
class SoupServings:
        ...
        def dp(x, y):
            if (x, y) not in memo:
                if x <= 0 or y <= 0:
                    ...
                    ans = 0.25 * (dp(x-4, y) + dp(x-3, y-1) +
                                  dp(x-2, y-2) + dp(x-1, y-3))
                memo[x, y] = ans
            return memo[x, y]
    ...
    def doit1(self, N):
        ...
        def dfs(A, B):
            ...
            p = 0
            for c in serves:
                c1 = (A - c[0]) if A > c[0] else 0
                c2 = (B - c[1]) if B > c[1] else 0
                p += dfs(c1, c2)
...
if __name__ == '__main__':
    ...
    res = SoupServings().doit1(N=50)  # 0.625

    res = SoupServings().doit1(N=101)  # 0.74219
