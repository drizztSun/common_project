...
class NumberOfLines(object):
    ...
    def doit(self, widths, S):
        sum_ = 0
        for s in S:
            if widths[ord(s)-ord('a')]>(100-sum_%100):
                sum_ += (100-sum_%100)
            sum_ += widths[ord(s)-ord('a')]

        if not sum_%100:
            return [sum_//100,0]
        else:
            return [sum_//100+1,sum_%100]


if __name__ == "__main__":

    res = NumberOfLines().doit(widths=[10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10],
                               S="abcdefghijklmnopqrstuvwxyz")

    res = NumberOfLines().doit(widths=[4, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10],
                               S="abcdefghijklmnopqrstuvwxyz")
