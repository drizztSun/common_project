...
class IsLongPressedName:
    def doit(self, name: str, typed: str) -> bool:

        def index(word):
            s, ans = -1, []
            for i in range(len(word)-1):
                if i == len(word) - 1 or word[i] != word[i+1]:
                    ans.append((word[i], i - s))
                    s = i
            return ans

        name_index = index(name)
        typed_index = index(typed)

        if len(name_index) != len(typed_index):
            return False

        return all(name_index[i][0] == typed_index[i][0] and name_index[i][1] <= typed_index[i][1] for i in range(len(name_index)))
    ...
    Time Complexity: O(N+T), where N, TN,T are the lengths of name and typed.

    Space Complexity: O(N+T).
    """

    def doit(self, name, typed):
...
if __name__ == '__main__':
    ...
    res = IsLongPressedName().doit(name="alex", typed="aaleex")

    res = IsLongPressedName().doit(name="saeed", typed="ssaaedd")

    res = IsLongPressedName().doit(name="leelee", typed="lleeelee")

    res = IsLongPressedName().doit(name="laiden", typed="laiden")
