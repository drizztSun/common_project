...
class LUSlength(object):
    def doit(self, strs):
        ...
        """
        ans=-1
        def subsequence(a, b): #judge if a is a subsequence of b
            if len(a) > len(b):
                return False
        ...
        for j in range(len(strs)):
            if all(not subsequence(strs[j], strs[k]) for k in range(len(strs)) if k!=j):
                    ans = max(ans, len(strs[j]))
