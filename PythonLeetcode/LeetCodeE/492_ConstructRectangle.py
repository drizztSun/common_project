...
class ConstructRectangle(object):
    def doit(self, area):
        """
        :type area: int
        :rtype: List[int]
        """        
        for i in reversed(range(1, math.sqrt(area) + 1)):
...
if __name__ == "__main__":
    ...
    res = ConstructRectangle().doit(4)
    
    res = ConstructRectangle().doit(6)

    res = ConstructRectangle().doit(7)