...
from order_data where rnk = 1


WITH CTE AS (SELECT O.order_id, O.order_date, O.customer_id,C.name, O.product_id, P.product_name, P.price FROM Orders O JOIN Customers C ON O.customer_id = C.customer_id JOIN Products P ON O.product_id = P.product_id)
SELECT customer_id, product_id, product_name FROM
(SELECT customer_id, product_id, product_name, RANK() OVER(PARTITION BY customer_id ORDER BY COUNT DESC) AS RK FROM
(SELECT customer_id, product_id, product_name, count(*) AS COUNT FROM CTE GROUP BY customer_id, product_id, product_name) A )B WHERE B.RK = 1